...
export OS_KEY=$(curl -s https://identity.api.rackspacecloud.com/v2.0/users/${USERID}/OS-KSADM/credentials/RAX-KSKEY:apiKeyCredentials -H "Content-Type: application/json" -H "X-Auth-Token: $OS_TOKEN" | python -m json.tool | python -c "import json;import sys;print '%s' % json.loads(sys.stdin.read())['RAX-KSKEY:apiKeyCredentials']['apiKey']")
...
#
source openstack_cli_functions
